...
> ⚠️ `switch_vllm.sh` 和 `switch_comfyui.sh` 已废弃。统一使用 `edge-llm` CLI。
...
EdgeLLM 管理单卡 GPU 上多个互斥/共存 LLM 推理服务和图像生成服务的生命周期。
...
| `edge-llm switch <profile>` | `edge-llm switch <model_name>` |
| 停止只能 switch idle | `edge-llm stop <model_name>` 停单个 |
...
# 在共享模式下加入 ComfyUI
edge-llm switch comfyui

# 停止单个共享服务
edge-llm stop qwen35-9b
...
├── scripts/
│   ├── edge-llm-recovery.sh         # 紧急恢复
│   ├── switch_vllm.sh.bak           # 已废弃(edge-llm CLI 替代)
│   └── switch_comfyui.sh            # 已废弃(edge-llm CLI 替代)
└── tests/
    ├── test_v4.py                   # v4.0 测试
...
> 当 proxy 故障时,CLI 是应急入口:`edge-llm status` / `edge-llm switch idle` / `edge-llm reconcile`。
...
过滤:`edge-llm models --mode exclusive`

### `edge-llm switch <model_name|idle>`

遵守三态规则。独占模型全锁 GPU,共享模型允许共存。

```bash
# 常用操作
edge-llm switch qwen36-27b    # 启动 Qwen3.6-27B(独占,自动停其他服务)
edge-llm switch comfyui       # 启动 ComfyUI(共享)
edge-llm switch qwen35-9b     # 加入 Qwen3.5-9B(与 ComfyUI 共存)
edge-llm switch idle          # 释放 GPU(停所有服务)
```

### `edge-llm stop <model_name>`

停止单个共享服务。其他共享服务保留。最后一个停止后自动转 idle。

```bash
edge-llm stop qwen35-9b       # 停 Qwen3.5-9B,保留 ComfyUI
```
...
# 4. 重启 proxy
python3 -m edge_llm serve
# 或 systemd
sudo systemctl restart edge-llm
...
1. 下载模型到 `~/models/`
2. 创建 conda 环境
3. 写 `~/edge_llm/models.d/new-model.yaml`
4. `edge-llm models` 可见 → `edge-llm switch new-model` 可用

### 删除模型

1. 删 `~/edge_llm/models.d/old-model.yaml`
